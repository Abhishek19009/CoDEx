...
class MultiUTAE(nn.Module):
    def __init__(
        ...
        self.all_coord_weights = nn.Parameter(torch.randn(num_domains, requires_grad=True))
        ...
        if self.feature_type == 'classifier':
            self.domain_classifier = DomainClassifier(in_channels=self.encoder_widths[-1], num_domains=num_domains)
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            for sits_idx, domain_i in enumerate(domains):
                ...
                coord_weight = F.cosine_similarity(coord_weight, self.all_coord_weights)
                
                coord_weight = coord_weight.view(1, -1)  # Shape (1,1)
                all_coord_weights = self.all_coord_weights.view(1, -1)  # Shape (1, 55)
                coord_weight = F.cosine_similarity(coord_weight, all_coord_weights, dim=1)
                coord_weight = coord_weight.squeeze()
            ...
            for sits_idx, domain_i in enumerate(domains):
                ...
                coord_weight = coord_weight.expand_as(self.all_coord_weights)  # Shape (1,1)
                coord_weight = F.cosine_similarity(coord_weight.unsqueeze(dim=1), self.all_coord_weights.unsqueeze(dim=1), dim=0)
                coord_weight = coord_weight.squeeze()
