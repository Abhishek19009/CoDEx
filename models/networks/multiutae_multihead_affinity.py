...
def masked_softmax(vector, mask):
    ...
    masked_vector[mask] = float('-inf')
    return torch.nn.functional.softmax(masked_vector, dim=0)
...
class MultiUTAE(nn.Module):
    def __init__(
        ...
        self.all_coord_weights = nn.Parameter(torch.randn(num_domains, num_domains, requires_grad=True))

        # Simple Linear to get coord score for patch x
        self.coord_regress = nn.ModuleList(
            [
                nn.Linear(in_features, 1)
            ]
        )
        # self.geo_weight.data = (similarity_matrix.data + similarity_matrix.data.T) / 2
        # self.geo_weight.data = torch.nn.functional.normalize(similarity_matrix.data, dim=1)
    ...
    def forward(self, batch):
        ...
        for l in self.coord_regress:
            out_c = out.mean(dim=1)
            out_c = out_c.reshape(batch_size, out_c.shape[1], -1)
            out_c = out_c.mean(dim=-1)
            coord_x = l(out_c)

        if self.feature_type == 'classifier':
            # Transform features for domain classification
            domain_logits = self.domain_classifier(out)
        ...
        if self.training:
            ...
            for sits_idx, domain_i in enumerate(domains):
                coord_weight = self.all_coord_weights[sits_idx]
                mask = torch.zeros_like(coord_weight, dtype=torch.bool)
                mask[domain_i] = True
                coord_weight = masked_softmax(coord_weight, mask)
                ...
                for domain_j in range(self.num_domains):
                    if domain_j != domain_i:
                        ...
                        domain_out = domain_out * coord_weight[domain_j]
                        out_logits_c[sits_idx] = out_logits_c[sits_idx] + domain_out

                # mask = torch.ones_like(coord_weight, dtype=torch.bool)
                # mask[domain_i] = False

                # out_logits_c[sits_idx] = out_logits_c[sits_idx].clone() / (coord_weight[mask].sum() + 1e-6)


            return {"out_logits_s": out_logits_s, "out_logits_c": out_logits_c}
        

        elif self.return_all_heads:
            out_preds = []
            # Pass the batch through all the heads. Return tensor of shape (55, BS, seq_len, C, H, W)
            for domain in range(self.num_domains):
                # domain_out, act = self.out_convs[domain](out.view(batch_size * seq_len, -1 , h, w)).view(
                #     batch_size, seq_len, -1, h, w
                # )
                domain_out = self.out_convs[domain](out.view(batch_size * seq_len, -1, h, w))
                domain_out = domain_out.view(batch_size, seq_len, -1, h, w)
                domain_out = domain_out.unsqueeze(0)
                # act = act.view(batch_size, seq_len, -1, h, w)
                
                # act = act.unsqueeze(0)
                # out_acts.append(act)
                out_preds.append(domain_out)

            return {"logits": torch.cat(out_preds), "domain_feats": domain_feats}
        

        elif not self.training:
            out_logits_s = [None for _ in range(len(domains))]
            for sits_idx, domain_i in enumerate(domains):
                coord_weight = coord_x[sits_idx]
                coord_weight = torch.abs(coord_weight - self.all_coord_weights[0])
                head_logits = []
                for domain_j in range(self.num_domains):
                    domain_out = self.out_convs[domain_j](out[sits_idx].view(1 * seq_len, -1, h, w)).view(
                        seq_len, -1, h, w
                    ...
                    head_logits.append(domain_out.view(1, seq_len, -1, h, w))
                    ...
                    if out_logits_s[sits_idx] is None:
                        ...
                        out_logits_s[sits_idx] = out_logits_s[sits_idx] + domain_out
                
                out_logits_s[sits_idx] = out_logits_s[sits_idx] / (coord_weight.sum() + 1e-6)
            
            out_logits_s = torch.stack(out_logits_s)
            return {"out_logits_s": out_logits_s, "out_logits_c": torch.zeros_like(out_logits_s)}
