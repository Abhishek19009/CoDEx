...
class MultiUTAE(nn.Module):
    ...
    def forward(self, batch):
        ...
        if not self.training and not self.return_all_heads:
            if self.feature_type == 'input':
                heads_idx = select_head(x, domains, split, self.feature_type)
            elif self.feature_type == 'learned':
                heads_idx = select_head(out, domains, split, self.feature_type)
            elif self.feature_type == 'classifier':
                heads_idx = torch.argmax(domain_logits, dim=-1)
            elif self.feature_type == 'aggregate':
                for idx in range(self.n_stages - 1):
                    out_preds.append(self.out_convs[idx](out.view(batch_size * seq_len, -1 , h, w)).view(
                        batch_size, seq_len, -1, h, w
                    ))
                return {"logits": torch.sum(torch.stack(out_preds), dim=0)}
            
            else:
                heads_idx = domains

            print(heads_idx, out.shape)
            
            for idx, domain in enumerate(heads_idx):
                domain_out = self.out_convs[domain](out[idx].view(1 * seq_len, -1, h, w)).view(
                    1, seq_len, -1, h, w
                )
                
                out_preds.append(domain_out)

            # return {"logits": torch.cat(out_preds), "domain_logits": domain_logits}
            return {"logits": torch.cat(out_preds)}
