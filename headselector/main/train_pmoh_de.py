...
def parse_args():
    ...
    parser.add_argument('--num_domains', type=int, default=55, help='Number of domains')
    parser.add_argument('--feat_type', type=str, default='encoder', help='Which blocks to select?')
    parser.add_argument('--label_type', type=str, default='acc', help="Which label type")
    ...
    parser.add_argument('--input_dim', type=int, default=4, help='Input dimension of the model')
    parser.add_argument('--in_features', type=int, default=64, help='Number of input features for the model')
    parser.add_argument('--seq_length', type=int, default=12, help='Sequence length for temporal data')
    ...
    parser.add_argument('--pretrained_checkpoint_path', type=str, default="../checkpoints/DynamicEarthNet_MultiHead_MultiUTAE_Learned/best_miou_ckpt_in.ckpt", help='Path to pretrained checkpoint')
    ...
    parser.add_argument('--ignore_index', type=int, default=6)

    parser.add_argument('--data_dir', type=str, default="../datasets/DynamicEarthNet_feats", help='Directory containing dataset')
...
if args.load_save:
    ...
    ckpt_path = os.path.join(args.checkpoint_path, f'best_{args.criterion}_{args.label_type}_{args.num_convs}_{args.feat_type}_{args.kernel_size}_{args.out_first}_classifier.ckpt')
...
def reset_optimizer(model, lr, betas=(0.9, 0.999), weight_decay=0.01):
    return torch.optim.Adam(model.parameters(), lr=lr, betas=betas, weight_decay=weight_decay)


wandb.init(project="arch_pmoh", entity="imaginelab", name=f"{args.criterion}_{args.label_type}_{args.num_convs}_{args.feat_type}_{args.kernel_size}_{args.out_first}_{args.sgd_momentum}", config=vars(args))
